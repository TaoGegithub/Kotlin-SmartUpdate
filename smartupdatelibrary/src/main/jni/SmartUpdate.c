//
// Created by longt on 2017/12/13.
//
#include <stdlib.h>
#include <stdio.h>
#include <string.h>
...
Java_com_tao_smartupdatelibrary_SmartUpdate_update(JNIEnv *env, jclass type, jstring oldPath_,
                                                   jstring newPath_, jstring patchPath_) {
    char * ch[4];
    ch[0] = "bspatch";
    ch[1] = (char*) ((*env)->GetStringUTFChars(env, oldPath_, 0));
    ch[2] = (char*) ((*env)->GetStringUTFChars(env, newPath_, 0));
    ...
    (*env)->ReleaseStringUTFChars(env, oldPath_, ch[1]);
    (*env)->ReleaseStringUTFChars(env, newPath_, ch[2]);
